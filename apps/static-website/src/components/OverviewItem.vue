<template>
  <article class="overview-card">
    ...
    <img
      width="20"
      height="20"
      class="overview-card__image"
      :src="getIcon(props.socialNetwork).value"
      alt=""
    />
    <span class="overview-card__counter">{{
      formatNumber(props.views, 0)
    }}</span>
    <div class="overview-card__indicator" :class="getClassIndicator">
      <img
        ...
        height="4"
        :alt="altImage"
        :src="getIndicator(props.percentage).value"
      />
...
<script setup>
import { formatNumber, stringToSlug } from "../utils/formatters";
...
const altImage = computed(() =>
  props.percentage >= 0
    ? "up arrow, views increase today"
    : "down arrow, views decrease today"
);

const hrefAccount = computed(
  () =>
    `${basePath(props.socialNetwork)}${stringToSlug(props.account)}/${stringToSlug(
      props.metric
    )}`
);

const cardDescription = computed(
  () => `${props.metric} of ${props.account} at ${props.socialNetwork}`
);
</script>

  ...
  <article :class="`dashboard__item dashboard__item--${socialNetwork}`">
    ...
    <div class="dashboard__indicator" :class="getClassIndicator">
      <img
        ...
        height="4"
        :alt="altImage"
        :src="getIndicator(props.views).value"
      />
...
const formatFollowers = computed(() => formatNumber(props.followers, 0));
...
const altImage = computed(() =>
  props.percentage >= 0
    ? "up arrow, percentage increase"
    : "down arrow, percentage decrease"
);

const hrefAccount = computed(
  () => basePath(props.socialNetwork) + stringToSlug(props.account)
);

const cardLinkDescription = computed(
  () => "go to " + props.account + " at " + props.socialNetwork
);
</script>
